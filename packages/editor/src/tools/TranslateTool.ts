...
export class TranslateTool extends BaseTool {
  ...
  private initializeGizmoHandles(): void {
    this.gizmoHandles = [
      // Axis handles
      { type: 'axis', axis: 'x', position: new Vector3(1, 0, 0), size: 0.2, color: [1, 0, 0] },
      { type: 'axis', axis: 'y', position: new Vector3(0, 1, 0), size: 0.2, color: [0, 1, 0] },
      { type: 'axis', axis: 'z', position: new Vector3(0, 0, 1), size: 0.2, color: [0, 0, 1] },
      // Plane handles
      { type: 'plane', axis: 'xy', position: new Vector3(0.3, 0.3, 0), size: 0.25, color: [1, 1, 0] },
      { type: 'plane', axis: 'xz', position: new Vector3(0.3, 0, 0.3), size: 0.25, color: [1, 0, 1] },
      { type: 'plane', axis: 'yz', position: new Vector3(0, 0.3, 0.3), size: 0.25, color: [0, 1, 1] },
    ];
  }
  ...
  private getHandleAtPosition(x: number, y: number): GizmoHandle | null {
    ...
    const entityPos = this.selectedEntity.transform?.position || { x: 0, y: 0, z: 0 };
  ...
  private renderAxisHandle(context: any, origin: any, handle: GizmoHandle, alpha: number): void {
    ...
    this.renderCone(context, endPos, handle.axis, [r, g, b, alpha], handle.size);
  }
  
  private renderPlaneHandle(context: any, origin: any, handle: GizmoHandle, alpha: number): void {
    const [r, g, b] = handle.color;
  ...
  private renderLine(context: any, start: any, end: any, color: number[], width: number): void {
    if (context.drawLine) {
      context.drawLine(start, end, color, width);
    }
  }
  
  private renderCone(context: any, position: any, axis: string, color: number[], size: number): void {
    if (context.drawCone) {
      context.drawCone(position, axis, color, size);
    }
  }
  
  private renderQuad(context: any, position: any, size: number, color: number[]): void {
    if (context.drawQuad) {
